// frontend/src/components/marketing/MarketingDashboard.jsx
...
function MarketingDashboard({ user, onLogout }) {
  ...
  const fetchMarketingReport = async () => {
    ...
    try {
      ...
      console.error(t('marketing.fetch_error'), error)
    } finally {
      setLoading(false)
    }
  }

  // Marketing menyusi (Director bilan bir xil)
  const menuItems = [
    { id: 'home', icon: '🏠', label: t('marketing.home') },
    { id: 'orders', icon: '📦', label: t('marketing.orders') },
    { id: 'clients', icon: '🏥', label: t('marketing.clients') },
    { id: 'payments', icon: '💰', label: t('marketing.payments') },
    { id: 'employees', icon: '👥', label: t('marketing.employees') },
    { id: 'products', icon: '💊', label: t('marketing.products') },
    { id: 'inventory', icon: '🏭', label: t('marketing.inventory') },
    { id: 'reports', icon: '📊', label: t('marketing.reports') }
  ]
  ...
  const renderContent = () => {
    switch (currentView) {
        ...
        return (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold text-gray-800">{t('marketing.dashboard_title')}</h1>
                <p className="text-gray-500 mt-1">{t('marketing.dashboard_subtitle')}</p>
              </div>
              <Button onClick={fetchMarketingReport} variant="outline">
                {t('buttons.refresh')}
              </Button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card padding="md" className="border-l-4 border-blue-500">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-sm text-gray-500 font-medium">{t('marketing.total_orders')}</p>
              ...
              <Card padding="md" className="border-l-4 border-green-500">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-sm text-gray-500 font-medium">{t('marketing.approved_orders')}</p>
              ...
              <Card padding="md" className="border-l-4 border-purple-500">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-sm text-gray-500 font-medium">{t('marketing.total_sales')}</p>
              ...
              <Card padding="md" className="border-l-4 border-red-500">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-sm text-gray-500 font-medium">{t('marketing.total_debt')}</p>
            ...
            <Card padding="md">
              <h3 className="text-lg font-bold text-gray-800 mb-4">{t('marketing.additional_stats')}</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                  <p className="text-sm text-yellow-800 font-medium">{t('marketing.pending_orders')}</p>
                  <p className="text-2xl font-bold text-yellow-700 mt-1">{report?.pending_orders || 0}</p>
                </div>
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-sm text-gray-800 font-medium">{t('marketing.rejected_orders')}</p>
                  <p className="text-2xl font-bold text-gray-700 mt-1">{report?.rejected_orders || 0}</p>
                </div>
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="text-sm text-blue-800 font-medium">{t('marketing.total_products')}</p>
      ...
      // ✅ Barcha komponentlarga readOnly={true} berilmoqda!
      case 'orders':
  ...
  if (loading && currentView === 'home') {
    return <Loading fullScreen text={t('marketing.loading')} />
  }
        ...
        <div className="p-6 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">{t('marketing.role_title')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{t('marketing.role_name')}</p>
            </div>
          </div>
